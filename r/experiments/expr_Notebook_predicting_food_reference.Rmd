...
The following script is adapted as it is from [here](https://www.r-bloggers.com/predicting-food-preferences-with-sparklyr-machine-learning/)

The purpose of this adaption is to learn how notebooks work in R.
...
Now lets check the installed spark version on local machine

```{r}
# check spark installed versions
spark_installed_versions()
```
As we can see there are 3 versions of spark installed on my local machine. Next, I will choose a version for further analysis. 
```{r}
...
Dividing the data into training and test sets using the sdf_partition() function.
